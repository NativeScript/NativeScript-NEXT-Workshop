    ...
    function CreateAnalyticsMonitor(settings) {
        ...
        (function () {
            try {
                ...
                environmentInfo = settings.environmentInfo;
        ...
        var createPayload = function (queryStringArray) {
            ...
            queryStringArray.push("pv=" + protocolVersion);
            queryStringArray.push("mv=" + monitorversion);
            ...
            if (payloadsSend === 0) {
                var isEnvInfoDefined = (typeof settings.environmentInfo !== 'undefined' && settings.environmentInfo !== null);
                payload.info = isEnvInfoDefined ? settings.environmentInfo : eqatecUtil.getInfo(logger, settings);
            }
                        ...
                        try {
                            if (4 == request.readyState) {
                                ...
                                if (success) {
                                    ...
                                    // Note: this statistics payload (FeatureUsage hit counts) will be lost due to the
                                    // current implementation. We should improve that by eg placing this particular payload
                                    // on a queue for retrying.
